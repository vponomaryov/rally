...
@base.context(name="manila", order=450)
class Manila(base.Context):
    ...
    def _setup_for_existing_users(self):
        ...
        for tenant_name_or_id, share_networks in self.config[
            ...
            manila = manila_utils.ManilaScenario(clients=clients)
            existing_sns = manila._list_share_networks(
    ...
    def _setup_for_autocreated_users(self):
        ...
        for user, tenant_id in (utils.iterate_per_tenants(
            ...
            data = {"name": manila._generate_random_name(
                prefix="rally_manila_share_network_")}

            def _setup_share_network(tenant_id, data):
                share_network = manila._create_share_network(**data)
                self.context["tenants"][tenant_id][
                    "share_networks"].append(share_network)
                if self.config["use_security_services"]:
                    for ss in self.config["security_services"]:
                        security_service = manila._create_security_service(
                            **ss)
                        self.context["tenants"][tenant_id][
                            "security_services"].append(security_service)
                        manila._add_security_service_to_share_network(
                            share_network.id, security_service.id)
    ...
    def cleanup(self):
        ...
        for user, tenant_id in (utils.iterate_per_tenants(
            ...
            manila = manila_utils.ManilaScenario(clients=clients)
            objects = [
                {"list": self.context["tenants"][tenant_id][
                    "share_networks"] or [],
                 "name": "share network",
                 "delete_func": manila._delete_share_network},
                {"list": self.context["tenants"][tenant_id][
                    "security_services"] or [],
                 "name": "security service",
                 "delete_func": manila._delete_security_service},
            ]
            for obj_set in objects:
                for obj in obj_set["list"]:
                    logger = log.ExceptionLogger(
                        LOG,
                        _("Failed to delete %(name)s %(id)s for tenant "
                          "%(t)s.") % {"id": obj, "t": tenant_id,
                                       "name": obj_set["name"]})
                    with logger:
                        obj_set["delete_func"](obj)
        ...
        for user, tenant_id in (utils.iterate_per_tenants(
            ...
            for sn in self.context["tenants"][tenant_id]["share_networks"]:
                share_servers = manila._list_share_servers(
