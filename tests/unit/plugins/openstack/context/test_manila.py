...
from rally.plugins.openstack.context import manila
...
class ManilaSampleGeneratorTestCase(test.TestCase):
    ...
    def _gen_context(self, use_security_services=False, networks_per_tenant=2,
    ...
    def test_init(self):
        ...
        inst = manila.Manila(context)

        self.assertEqual(inst.config, context["config"]["manila"])
    ...
    def test_setup_use_existing_share_networks(self, list_sns, osclients):
        existing_sns = self.existing_sns
        inst = manila.Manila(self.ctxt_use_existing)
    ...
    def test_setup_autocreate_share_networks_with_security_services(
            self, neutron, add_ss_to_sn, create_ss, create_sn, osclients):
        networks_per_tenant = 2
        ctxt = self._gen_context(
            networks_per_tenant=networks_per_tenant,
    ...
    def test_setup_autocreate_share_networks_wo_networks(
            self, add_ss_to_sn, create_ss, create_sn, osclients):
        ctxt = self._gen_context(networks_per_tenant=0)
        inst = manila.Manila(ctxt)
    ...
    def test_cleanup_used_existing_share_networks(
            self, list_sns, list_servers, delete_ss, delete_sn, osclients):
        inst = manila.Manila(self.ctxt_use_existing)
        list_sns.return_value = self.existing_sns
        inst.setup()
        ...
        self.assertEqual(2, osclients.call_count)
        for user in self.ctxt_use_existing["users"]:
            self.assertIn(mock.call(user["endpoint"]), osclients.mock_calls)
    ...
    def test_cleanup_autocreated_share_networks(
            ...
            create_sn, delete_ss, delete_sn, osclients, wait_for_delete):
